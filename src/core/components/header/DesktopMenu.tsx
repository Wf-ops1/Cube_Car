...
export const DesktopMenu: React.FC<DesktopMenuProps> = ({
    ...
    // Get dynamic sections and inject real badge count
    const sections = user ? getProfileMenuSections(user).map(section => ({
        ...section,
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className={`w-[320px] bg-white/95 backdrop-blur-3xl rounded-[28px] shadow-[0_30px_90px_-20px_rgba(0,0,0,0.15),0_0_0_1px_rgba(0,0,0,0.02)] border border-white/60 z-[${Z_INDEX.DROPDOWN}] origin-top antialiased`}
                    >
                        {!user ? (
                            /* GUEST MENU */
                            <motion.div
                            ...
                            >
                                ...
                                <motion.div
                                    variants={{ open: { opacity: 1, y: 0 }, closed: { opacity: 0, y: -10 } }}
                                    className="p-4 bg-slate-50/50 border-b border-slate-100/50 mb-0 cursor-pointer group"
                                    onClick={() => { onClose(); onEditProfileClick && onEditProfileClick(); }}
                                >
                                    <div className="flex items-center gap-4">
                                        <div className="relative">
                                            <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-white shadow-md group-hover:scale-105 transition-transform duration-500">
                                                {userAvatar ? (
                                ...
                                <div className="pt-0 pb-2">
                                    {sections.map((section, idx) => (
                                        <div key={section.title} className={`px-3 ${idx > 0 ? 'mt-2 pt-2 border-t border-slate-100/50' : ''}`}>
                                            {section.title && <span className="px-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-1 block">{section.title}</span>}
                                                ...
                                                {section.items.map((item) => (
                                                    item.id === 'host' ? (
                                                        ...
                                                        <motion.button
                                                            key={item.id}
                                                            variants={{ open: { opacity: 1, x: 0 }, closed: { opacity: 0, x: -10 } }}
                                                            onClick={() => item.id === 'logout' ? (onLogout && onLogout()) : handleMenuClick(item.id)}
                                                            className={`w-full flex items-center gap-3 py-1.5 transition-all duration-300 group text-left ${item.id === 'logout' ? '-mx-3 px-7 rounded-none mt-2 border-t border-slate-100 pt-3 hover:bg-transparent' : `px-4 rounded-xl ${item.highlight ? 'bg-primary/[0.03] hover:bg-primary/[0.06]' : 'hover:bg-slate-50'}`}`}
                                                        >
                                                            <div className={`flex items-center justify-center transition-all duration-500 ${item.id === 'logout' ? 'w-8 bg-transparent text-slate-400 group-hover:text-red-500' : `w-8 h-8 rounded-lg ${item.highlight ? 'bg-primary/10 text-primary scale-105' : 'bg-transparent text-slate-500 group-hover:text-slate-600'}`}`}>
                                                                <i className={`fas ${item.icon} text-[13px]`}></i>
                                                            </div>
                                                            <div className="flex flex-col flex-1">
                                                                <span className={`text-[14px] font-semibold tracking-tight transition-colors ${item.id === 'logout' ? 'text-slate-500 group-hover:text-red-500' : (item.highlight ? 'text-primary' : 'text-slate-900 group-hover:text-slate-900')}`}>{item.label}</span>
